...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for key in ['unpaired']:
            ...
            cm_name = c_name[key].replace(".jpg", ".jpg")
            c[key] = Image.open(osp.join(self.data_path, 'cloth', c_name[key])).convert('RGB')
            c[key] = transforms.Resize(self.fine_width, interpolation=2)(c[key])
            cm[key] = Image.open(osp.join(self.data_path, 'cloth-mask', cm_name))
        ...
        # load pose points
        # pose_name = im_name.replace('image', 'openpose_img').replace('.jpg', '_rendered.jpg')
