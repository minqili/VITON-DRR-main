...
class Pix2PixHDModel(BaseModel):
    ...
    def inference(self, label, pre_clothes_mask, img_fore, clothes_mask, clothes, all_clothes_label, real_image, pose, grid, mask_fore, name):
        ...
        G_in = torch.cat([img_hole_hand, dis_label, Warped_tensor,
                          skin_color, self.gen_noise(shape)], 1)
        fake_image = self.G.refine(G_in.detach())
        fake_image = self.tanh(fake_image)

        return [fake_image, Warped_tensor_temp, Warped_tensor]
...
class InferenceModel(Pix2PixHDModel):
    def forward(self, label, pre_clothes_mask, img_fore, clothes_mask, clothes, all_clothes_label, real_image, pose, grid, mask_fore, name):
        return self.inference(label, pre_clothes_mask, img_fore, clothes_mask, clothes, all_clothes_label, real_image, pose, grid, mask_fore, name)
